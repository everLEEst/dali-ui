...
Ui::AtlasManager::AtlasId AtlasManager::CreateAtlas(const Ui::AtlasManager::AtlasSize& size, Pixel::Format pixelformat)
{
  ...
  if(blockWidth + DOUBLE_PIXEL_PADDING + 1u > width || blockHeight + DOUBLE_PIXEL_PADDING + 1u > height)
  {
    DALI_LOG_ERROR("Atlas %i x %i too small. Dimensions need to be at least %ix%i\n", width, height,
                   blockWidth + DOUBLE_PIXEL_PADDING + 1u, blockHeight + DOUBLE_PIXEL_PADDING + 1u);
    return 0;
  }
  ...
  PixelData backgroundPixels =
    PixelData::New(background, bufferSize, width, height, pixelformat, PixelData::DELETE_ARRAY);
  ...
  atlasDescriptor.mHorizontalStrip = PixelData::New(bufferHorizontalStrip, bufferSize, blockWidth, SINGLE_PIXEL_PADDING,
                                                    pixelformat, PixelData::DELETE_ARRAY);
  ...
  atlasDescriptor.mVerticalStrip =
    PixelData::New(bufferVerticalStrip, bufferSize, SINGLE_PIXEL_PADDING, blockHeight - DOUBLE_PIXEL_PADDING,
                   pixelformat, PixelData::DELETE_ARRAY);
...
bool AtlasManager::Add(const PixelData& image, Ui::AtlasManager::AtlasSlot& slot, Ui::AtlasManager::AtlasId atlas)
{
  ...
  if(0u == foundAtlas)
  {
    if(Ui::AtlasManager::FAIL_ON_ADD_CREATES == mAddFailPolicy)
    {
      ...
      {
        foundAtlas =
          CreateAtlas(mNewAtlasSize, pixelFormat); // Creating atlas with mNewAtlasSize, may not be the needed size!
        if(0u == foundAtlas)
        {
          DALI_LOG_ERROR("Failed to create an atlas of %i x %i blocksize: %i x %i.\n", mNewAtlasSize.mWidth,
                         mNewAtlasSize.mHeight, mNewAtlasSize.mBlockWidth, mNewAtlasSize.mBlockHeight);
          return false;
        }
        else
        {
...
AtlasManager::SizeType AtlasManager::CheckAtlas(SizeType atlas, SizeType width, SizeType height,
                                                Pixel::Format pixelFormat)
{
  ...
  if(pixelFormat == mAtlasList[atlas].mPixelFormat)
  {
    ...
    const SizeType availableBlocks =
      mAtlasList[atlas].mAvailableBlocks + static_cast<SizeType>(mAtlasList[atlas].mFreeBlocksList.Size());

    if(availableBlocks &&
       IsBlockSizeSufficient(width, height, mAtlasList[atlas].mSize.mBlockWidth, mAtlasList[atlas].mSize.mBlockHeight))
    {
...
void AtlasManager::UploadImage(const PixelData& image, const AtlasSlotDescriptor& desc)
{
  ...
  DALI_ASSERT_DEBUG(horizontalStripHeight == SINGLE_PIXEL_PADDING && verticalStripWidth == SINGLE_PIXEL_PADDING &&
                    "Strip buffer size invalid!");
  ...
  // Blit top 2 strip (Note that we should not blit the 0,0 pixel since it is special color for underline and
  // strikethrough)
  if(horizontalStripWidth > firstRowBlitOffset)
  {
    uploaded =
      mAtlasList[atlas].mAtlas.Upload(mAtlasList[atlas].mHorizontalStrip, 0u, 0u, blockOffsetX + firstRowBlitOffset,
                                      blockOffsetY, horizontalStripWidth - firstRowBlitOffset, horizontalStripHeight);
  }
  uploaded &=
    mAtlasList[atlas].mAtlas.Upload(mAtlasList[atlas].mHorizontalStrip, 0u, 0u, blockOffsetX,
                                    blockOffsetY + SINGLE_PIXEL_PADDING, horizontalStripWidth, horizontalStripHeight);
  DALI_ASSERT_DEBUG(uploaded && "Uploading top strip to Atlas Failed!");

  // Blit left 2 strip
  uploaded = mAtlasList[atlas].mAtlas.Upload(mAtlasList[atlas].mVerticalStrip, 0u, 0u, blockOffsetX, imageOffsetY,
                                             verticalStripWidth, verticalStripHeight);
  uploaded &=
    mAtlasList[atlas].mAtlas.Upload(mAtlasList[atlas].mVerticalStrip, 0u, 0u, blockOffsetX + SINGLE_PIXEL_PADDING,
                                    imageOffsetY, verticalStripWidth, verticalStripHeight);
  ...
  {
    uploaded = mAtlasList[atlas].mAtlas.Upload(mAtlasList[atlas].mHorizontalStrip, 0u, 0u, blockOffsetX,
                                               blockOffsetY + height + DOUBLE_PIXEL_PADDING, horizontalStripWidth,
                                               horizontalStripHeight);
    if(blockOffsetY + height + TRIPLE_PIXEL_PADDING < mAtlasList[atlas].mSize.mHeight)
    {
      uploaded &= mAtlasList[atlas].mAtlas.Upload(mAtlasList[atlas].mHorizontalStrip, 0u, 0u, blockOffsetX,
                                                  blockOffsetY + height + TRIPLE_PIXEL_PADDING, horizontalStripWidth,
                                                  horizontalStripHeight);
    }
  ...
  {
    uploaded = mAtlasList[atlas].mAtlas.Upload(mAtlasList[atlas].mVerticalStrip, 0u, 0u,
                                               blockOffsetX + width + DOUBLE_PIXEL_PADDING, imageOffsetY,
                                               verticalStripWidth, verticalStripHeight);
    if(blockOffsetX + width + TRIPLE_PIXEL_PADDING < mAtlasList[atlas].mSize.mWidth)
    {
      uploaded &= mAtlasList[atlas].mAtlas.Upload(mAtlasList[atlas].mVerticalStrip, 0u, 0u,
                                                  blockOffsetX + width + TRIPLE_PIXEL_PADDING, imageOffsetY,
                                                  verticalStripWidth, verticalStripHeight);
    }
                                    ...
                                    bool addReference)
{
  if(id)
  {
    ...
    AtlasMeshFactory::CreateQuad(width, height, mImageList[imageId].mBlock, mAtlasList[atlas].mSize, position,
                                 meshData);
...
void AtlasManager::GetMetrics(Ui::AtlasManager::Metrics& metrics)
{
  ...
  for(uint32_t i = 0; i < atlasCount; ++i)
  {
    ...
    entry.mBlocksUsed  = entry.mTotalBlocks - mAtlasList[i].mAvailableBlocks +
                        static_cast<SizeType>(mAtlasList[i].mFreeBlocksList.Size());
