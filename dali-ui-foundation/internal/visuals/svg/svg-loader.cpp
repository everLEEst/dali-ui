...
void SetTextureSetToRasterizeInfo(const Dali::PixelData rasterizedPixelData, SvgLoader::SvgRasterizeInfo& rasterizeInfo)
{
  ...
  Dali::Texture texture = Texture::New(Dali::TextureType::TEXTURE_2D, Pixel::RGBA8888, rasterizedPixelData.GetWidth(),
                                       rasterizedPixelData.GetHeight());
#if defined(GPU_MEMORY_PROFILE_ENABLED)
  Dali::Integration::TextureUploadWithContent(texture, rasterizedPixelData, ToDaliString(rasterizeInfo.mImageUrl.GetUrl()), Dali::Integration::TextureContextTypeHint::SVG_IMAGE);
#else
                                     ...
                                     bool synchronousLoading)
{
  ...
  if(cacheIndex == SvgLoader::INVALID_SVG_CACHE_INDEX)
  {
    ...
    DALI_LOG_INFO(gSvgLoaderLogFilter, Debug::General,
                  ...
                  dpi, svgObserver, cacheIndex, loadId);
  }
  else
  {
                                               ...
                                               SvgLoaderObserver* svgObserver, bool synchronousLoading)
{
...
#endif
    DALI_LOG_INFO(gSvgLoaderLogFilter, Debug::General,
                  "SvgLoader::Rasterize( loadId=%d Size=%ux%u observer=%p ) New cached index:%d rasterizeId@%d\n",
  ...
  else
  {
    ...
    DALI_LOG_INFO(gSvgLoaderLogFilter, Debug::General,
                  "SvgLoader::Rasterize( loadId=%d Size=%ux%u observer=%p ) Use cached index:%d rasterizeId@%d\n",
                  loadId, width, height, svgObserver, cacheIndex, rasterizeId);
  }
...
SvgLoader::SvgCacheIndex SvgLoader::FindCacheIndexFromRasterizeCache(const SvgLoadId loadId, uint32_t width,
...
void SvgLoader::NotifyLoadObservers(SvgLoader::SvgLoadInfo& loadInfo)
{
  ...
  while(info->mObservers.Count())
  {
    ...
    DALI_LOG_INFO(gSvgLoaderLogFilter, Debug::Concise,
                  "SvgLoader::NotifyLoadObservers() observer:%p loadId:%d url:%s loadState:%s\n", observer, loadId,
                  info->mImageUrl.GetUrl().c_str(), GET_LOAD_STATE_STRING(info->mLoadState));
...
void SvgLoader::RasterizeRequest(SvgLoader::SvgRasterizeInfo& rasterizeInfo, SvgLoaderObserver* svgObserver)
{
  ...
  if(mLoadCache[loadCacheIndex].mTask)
  {
    ...
    mLoadCache[loadCacheIndex].mTask->AddNotifyObservedTaskList(rasterizingTask);
  }
...
void SvgLoader::RasterizeSynchronously(SvgLoader::SvgRasterizeInfo& rasterizeInfo, SvgLoaderObserver* svgObserver)
{
  ...
  // Note, we will not store this task after this API called.
  SvgRasterizingTaskPtr rasterizingTask = new SvgRasterizingTask(vectorImageRenderer, rasterizeInfo.mId,
                                                                 rasterizeInfo.mWidth, rasterizeInfo.mHeight, nullptr);
#ifdef TRACE_ENABLED
  {
    auto loadCacheIndex = GetCacheIndexFromLoadCacheById(rasterizeInfo.mLoadId);
    rasterizingTask->SetUrl(mLoadCache[loadCacheIndex].mImageUrl);
...
void SvgLoader::AsyncRasterizeComplete(SvgTaskPtr task)
{
  ...
  if(cacheIndex != SvgLoader::INVALID_SVG_CACHE_INDEX)
  {
    ...
    DALI_LOG_INFO(gSvgLoaderLogFilter, Debug::Concise,
                  "  rasterizeId:%d loadId:%d Size:%ux%u CacheIndex:%d RasterizeState: %s\n", rasterizeInfo.mId,
