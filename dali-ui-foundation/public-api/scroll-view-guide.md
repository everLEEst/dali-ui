...
Calling `SetContent()` again replaces the previous content. The old content is automatically removed from the scroll view.

```cpp
View newContent = View::New();
newContent.SetRequestedWidth(800.0f);
newContent.SetRequestedHeight(1200.0f);

scrollView.SetContent(newContent);  // previous content is removed automatically
```
...
### Practical example: collapsing header
